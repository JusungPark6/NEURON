...
def mcVolt(voltInput, length, startS, AMPAc):
	...
	print("test 1.5")
	savestate = loadSv(length,v_init*-1)
	# Loads the savestate to a variable 'savestate'
	print("test 2")
	# Running the test
	runTh(length,cell,startS,AMPAc, v_init)


def runTh(nSpines, cell, startS, AMPAc, v_init):
	...
	h.sprint(ThName,"Tr%dThLen%dV%dB.dat",startS,nSpines,v_init*-1)
	h.ThreshSpace(50,50,10,10,nSpines,0,ThName,startS,cell,AMPAc)
	h.sprint(ThName,"Tr%dThLen%dV%dA.dat",startS,nSpines,v_init*-1)
	h.ThreshSpace(50,50,10,10,nSpines,1,ThName,startS,cell,AMPAc)
	h.sprint(ThName,"Tr%dThLen%dV%dN.dat",startS,nSpines,v_init*-1)
